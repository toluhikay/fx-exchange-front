...
const Deposit = () => {
  const [depositMutation, { isLoading, isSuccess }] = QueryApi.useDepositMutation();
  ...
  const onSubmit: SubmitHandler<FormValues> = async (data) => {
    try {
      ...
      toast.success("Deposit successfult");
      reset();
    ...
    <div className="my-2 w-full">
      <form action="" onSubmit={handleSubmit(onSubmit)} className="w-full flex flex-col gap-y-3 justify-center items-center">
        ...
        <div className="w-fit border border-white rounded-lg overflow-hidden">
          <GeneralButton text="Deposit" loaderComponent={<ButtonLoader />} loading={isLoading} />
        </div>{" "}
